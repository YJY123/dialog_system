...
# -*- coding: utf-8 -*-
'''
FilePath: /dialog_system/ranking/similarity.py
...
from retrieval.hnsw_faiss import wam_similar
...
class TextSimilarity(object):
    ...
    def tokenSimilarity(self, str_a, str_b, method='w2v', sim='cos'):
        ...
        if method == 'w2v':
            vec_a = wam_similar(str_a, self.w2v_model)
            vec_b = wam_similar(str_b, self.w2v_model)
            model = self.w2v_model
        elif method == 'fasttest':
            vec_a = wam_similar(str_a, self.fasttext)
            vec_b = wam_similar(str_b, self.fasttext)
            model = self.fasttext
        elif method == 'tfidf':
            ...
            vocab_size = len(self.dictionary.token2id)
            vec_a = matutils.corpus2dense([vec_a], vocab_size).reshape(vocab_size,)
            vec_b = matutils.corpus2dense([vec_b], vocab_size).reshape(vocab_size,)
        else:
            NotImplementedError
        result = None
...
if __name__ == "__main__":
    ts = TextSimilarity()
    ts.tokenSimilarity("怎么更改还款时间", "不能呀部分还钱吗？", method='tfidf', sim='pearson')