...
# -*- coding: utf-8 -*-
'''
FilePath: /dialog_system/generative/train_distil.py
...
def main():
    ...
    if args.do_train:
        ...
        num_train_steps = len(trainloader) * args.num_train_epochs
...
def predict(model, data_path, step, args):
    ...
    with torch.no_grad():
        for (token_ids, token_type_ids, target_ids) in devloader:
            ...
            #                           token_type_ids,
            #                           labels=target_ids)
            enc_layers, logits, loss, attention_layers = model(token_ids,
                                                               token_type_ids,
                                                               labels=target_ids)
    ...
    epoch_loss = total_loss / len(devloader)
