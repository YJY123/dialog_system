...
# -*- coding: utf-8 -*-
'''
FilePath: /dialog_system/generative/seq2seq.py
...
class Seq2SeqModel(nn.Module):
    ...
    def beam_search(self,
        ...
        for step in range(self.out_max_length):
            ...
            loc_index = []   # 总感觉这部分代码贪心学院实现的有问题（新预测出来的字符应该是按照顺序拼接的，不应该原封不动直接拼接）
            ...
            if beam_size < 1:
                break
            # 连接(if判断是我对感觉错误的部分进行的修正)
            if step != 0:
                cat_tensor = []
                for i in loc_index:
                    cat_tensor.append(token_ids[i].view(1, -1))
                token_ids = torch.cat(cat_tensor)

            token_ids = torch.cat((token_ids, next_chars), dim=1)
            token_type_ids = torch.cat((token_type_ids,
                                        next_token_type_ids), dim=1)

        # 如果达到最大长度的话 直接把得分最高的输出序列返回
        # return output_ids[output_scores.argmax().item()]
        return output_ids[output_scores.argmax().item()][: -1]
